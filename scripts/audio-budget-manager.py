...
class AudioBudgetManager:
    ...
    def add_to_queue(self, doc_path: str, provider: str, priority: str, estimated_chars: int):
        """Add job to queue"""
        job = QueuedJob(
    ...
    def status_report(self) -> str:
        ...
        if self.active_provider in self.budgets:
            ...
            monthly_bar = self._progress_bar(tracker.monthly_used, budget["monthly_limit"])
            lines.append(f"📅 Monthly Usage ({tracker.month}):")
            lines.append(f"  {monthly_bar} {monthly_pct:.1f}%")
            lines.append(f"  Used: {tracker.monthly_used:,} / {budget['monthly_limit']:,} chars")
            lines.append(f"  Remaining: {budget['monthly_limit'] - tracker.monthly_used:,} chars")
            lines.append("")
            ...
            lines.append(f"  Used: {tracker.daily_used:,} / {budget['daily_limit']:,} chars")
            lines.append(f"  Remaining: {budget['daily_limit'] - tracker.daily_used:,} chars")
        ...
        if queued:
            ...
            for job in queued[:5]:
                lines.append(f"  [{job.priority.upper()}] {Path(job.doc_path).name} ({job.estimated_chars:,} chars)")
...
def main():
    ...
    if command == "status":
        ...
        if len(sys.argv) < 3:
            print("Usage: audio-budget-manager.py queue <doc.md> --provider <provider> --priority <priority>")
            sys.exit(1)

        doc_path = sys.argv[2]
        provider = sys.argv[sys.argv.index("--provider") + 1] if "--provider" in sys.argv else manager.active_provider
        priority = sys.argv[sys.argv.index("--priority") + 1] if "--priority" in sys.argv else "medium"
