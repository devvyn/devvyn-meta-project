...
class StoryParser:
    ...
    def validate_tags(self) -> bool:
        ...
        for tag in tags:
            ...
            if not re.match(r"^(NARRATOR|CHARACTER_\w+|CODE|QUOTE|HEADER)(\s+\w+=.+)?$", tag):
        ...
        if len(opening_tags) != len(closing_tags):
            print(f"Warning: Mismatched voice tags ({len(opening_tags)} opening, {len(closing_tags)} closing)")
            return False
